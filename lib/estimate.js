import { estimateReplacementCost } from './replacement-cost';
...
function homeEstimate(intake, profile) {
  ...
  const replacement = estimateReplacementCost(intake);
  let factor = replacement.midpoint / 425000;
  const reasons = [];
...
export function estimateAll(carriers, searchType, intake, profile) {
  return carriers
    .filter(c => {
      const productMatch = searchType === 'auto' ? c.autoIndex != null : searchType === 'home' ? c.homeIndex != null : (c.autoIndex != null && c.homeIndex != null);
      const occupancyMatch = !c.homeOccupancies || c.homeOccupancies.includes(intake.occupancy);
      return productMatch && occupancyMatch;
    })
